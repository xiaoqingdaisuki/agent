...
普通解释、翻译、写作、计划、比较和代码请求会走不携带工具 schema 的轻量模型路径；实时信息、计算、文件、知识库、记忆、推荐及未知意图保守保留完整工具链。高频固定问答可直接本地返回。Agent token 预算统一为：最大上下文 128k、最大输入 48k、最大输出 16k；历史上下文按输入预算并从完整用户轮次边界开始保留。
