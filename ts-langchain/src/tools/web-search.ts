...
function getSettings(): SearchSettings {
  return {
    ...
    timeoutMs: boundedInteger(process.env.SEARCH_TIMEOUT_MS, 4_500, 1_000, 15_000),
    maxResults: boundedInteger(process.env.SEARCH_MAX_RESULTS, 8, 1, 20),
    cacheTtlMs: boundedInteger(process.env.SEARCH_CACHE_TTL_SECONDS, 30, 0, 3_600) * 1_000,
    staleTtlMs: boundedInteger(process.env.SEARCH_STALE_TTL_SECONDS, 600, 0, 86_400) * 1_000,
    searchDepth: process.env.TAVILY_SEARCH_DEPTH === "advanced" ? "advanced" : "basic",
  };
}
...
async function fetchTavily(settings: SearchSettings, query: string): Promise<Response> {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      ...
      if (!RETRYABLE_STATUSES.has(response.status)) throw new NonRetryableSearchError(error.message);
...
async function executeSearch(query: string, cacheKey: string, settings: SearchSettings): Promise<SearchOutcome> {
  ...
  if (!settings.apiKey) {
    return { results: [], providers: [], failedProviders: ["tavily:not_configured"], cached: false, stale: false };
  }
  if (circuit.openUntil > now) {
    if (cached && cached.staleUntil > now) return { ...cached.outcome, cached: true, stale: true };
    return { results: [], providers: [], failedProviders: ["tavily:circuit_open"], cached: false, stale: false };
  }

  const startedAt = Date.now();
  try {
    const results = await searchTavily(query, settings);
    if (results.length === 0) throw new Error("Tavily returned an empty result set");
    ...
    console.info(JSON.stringify({ event: "web_search_succeeded", provider: "tavily", duration_ms: Date.now() - startedAt, result_count: outcome.results.length }));
    return outcome;
  } catch (error) {
    circuit.failures += 1;
    if (circuit.failures >= CIRCUIT_FAILURE_THRESHOLD) circuit.openUntil = Date.now() + CIRCUIT_COOLDOWN_MS;
    const reason = error instanceof Error ? error.message : "unknown error";
    console.warn(JSON.stringify({ event: "web_search_failed", provider: "tavily", duration_ms: Date.now() - startedAt, reason, consecutive_failures: circuit.failures }));
    if (cached && cached.staleUntil > now) return { ...cached.outcome, cached: true, stale: true };
    return { results: [], providers: [], failedProviders: [`tavily:${reason}`], cached: false, stale: false };
  }
}
...
export async function multiSourceSearch(query: string): Promise<SearchOutcome> {
  ...
  const now = Date.now();
  const cached = cache.get(cacheKey);
  if (cached && cached.freshUntil > now) return { ...cached.outcome, cached: true, stale: false };

  const running = inFlight.get(cacheKey);
  if (running) return running;
  const request = executeSearch(normalizedQuery, cacheKey, settings).finally(() => inFlight.delete(cacheKey));
  inFlight.set(cacheKey, request);
  return request;
}

export function searchResultsToText(outcome: SearchOutcome, query: string): string {
  if (outcome.results.length === 0) {
    const reason = outcome.failedProviders.join(", ");
    return `Tavily 实时搜索暂时不可用（${reason}）。不要把训练数据描述为实时结果；请告知用户稍后重试。`;
  }
  const freshness = outcome.stale ? "⚠️ Tavily 暂时不可用，以下为降级缓存结果" : "Tavily 实时搜索结果";
  const lines: string[] = [`🔍 ${freshness}（${query}）— 共 ${outcome.results.length} 条：\n`];
...
export const webSearchDescriptor: ToolDescriptor = {
  ...
  input_schema: {
    type: "object",
    properties: { query: { type: "string", description: "简洁明确的搜索关键词" } },
    required: ["query"],
  },
};
...
export const webSearchTool: DynamicStructuredTool = new DynamicStructuredTool({
  name: "web_search",
  description: "使用 Tavily 搜索实时互联网信息并返回来源链接。涉及新闻、当前数据、政策、人物或公司动态时必须使用。",
  schema: z.object({ query: z.string().trim().min(1).max(400).describe("简洁明确的搜索关键词") }),
  func: async ({ query }) => searchResultsToText(await multiSourceSearch(query), query),
});
